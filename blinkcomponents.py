...
class Tile(QtWidgets.QGraphicsRectItem):
    def __init__(self, idx, grid, pos):
        ...
        self.id = idx
        self.pos = pos
        self.setBrush(QtGui.QColor.fromRgb(*getcolor("tile_color")))
        pen = QtGui.QPen(QtGui.QColor.fromRgb(*getcolor("grid_line_color")))
...
class Grid(QtCore.QObject):
    ...
    def __init__(self, widget_size, scene, grid_size, frames=None):
        ...
        if frames is None:
            self.frames = [Frame(getint("frame_duration"), [getcolor("tile_color")] * (len(self.tiles)))]
        else:
            self.frames = frames
        self.current_color = getcolor("pen_color")
    ...
    def resize_tiles(self, size, item_size=None):
        ...
        for t in self.tiles:
            t.setRect(t.pos.x*item_size, t.pos.y*item_size, item_size-getint("grid_width"),
                      item_size-getint("grid_width"))
    ...
    def create_new_frame(self, duration=getint("frame_duration")):
        new_frame = Frame(duration, [getcolor("tile_color")] * (len(self.tiles)))
    ...
    def insert_frame_with_colors(self, frame_colors, pos=None):
        f = Frame(getint("frame_duration"), frame_colors)
    ...
    def delete_selected(self):
        for t in self.tiles:
            if t.isSelected():
                t.set_color(getcolor("tile_color"))
        self.select_all_tiles(False)
        ...
        for y in range(0, self.grid_size.height):
            for x in range(0, self.grid_size.width):
                ...
                if r.pos.x == self.grid_size.width - 1:
                    r.set_color(getcolor("tile_color"))
